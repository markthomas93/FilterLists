...
namespace FilterLists.Api.V3
{
    ...
    public class ProductsController : ODataController
    {
        ...
        {
            return SingleResult.Create(products.Where(p => p.Id == key));
        }
        ...
        public IActionResult Patch([FromODataUri] int key, Delta<Product> delta)
        {
            ...
            var product = new Product {Id = key, Name = "Updated Product " + key};
        ...
        {
            return SingleResult.Create(products.Where(p => p.Id == key).Select(p => p.Supplier));
        }
        ...
        public IActionResult GetRefToSupplier([FromODataUri] int key, [FromODataUri] string navigationProperty)
        {
            ...
            var property = entitySet.NavigationPropertyBindings.Single(p => p.Path.Path == navigationProperty)
                .NavigationProperty;
        ...
        [ProducesResponseType(Status204NoContent)]
        [ProducesResponseType(Status404NotFound)]
        public IActionResult CreateRefToSupplier([FromODataUri] int key, [FromODataUri] string navigationProperty,
            [FromBody] Uri link)
        {
            return NoContent();
        }
        ...
        [ProducesResponseType(Status204NoContent)]
        [ProducesResponseType(Status404NotFound)]
        public IActionResult DeleteRefToSupplier([FromODataUri] int key, [FromODataUri] string navigationProperty)
        {
            return NoContent();
        }
        ...
        private static Product NewProduct(int id)
        {
            return new Product
            {
                ...
                Name = "Product " + id,
                Price = id,
                Supplier = new Supplier {Id = id, Name = "Supplier " + id},
                SupplierId = id
            };
        }
    }
