...
namespace FilterLists.Api.V3
{
    ...
    public class SuppliersController : ODataController
    {
        ...
        {
            return SingleResult.Create(suppliers.Where(p => p.Id == key));
        }
        ...
        public IActionResult Patch([FromODataUri] int key, Delta<Supplier> delta)
        {
            ...
            var supplier = new Supplier {Id = key, Name = "Updated Supplier " + key};
        ...
        {
            return suppliers.Where(s => s.Id == key).SelectMany(s => s.Products);
        }
        ...
        [ProducesResponseType(Status204NoContent)]
        [ProducesResponseType(Status404NotFound)]
        public IActionResult CreateRefToProducts([FromODataUri] int key, [FromODataUri] string navigationProperty,
            [FromBody] Uri link)
        {
            return NoContent();
        }
        ...
        [ProducesResponseType(Status204NoContent)]
        [ProducesResponseType(Status404NotFound)]
        public IActionResult DeleteRefToProducts([FromODataUri] int key, [FromODataUri] string relatedKey,
